...
_.getPageTitle = function (e) {
    ...
    _.extend(a, this.store.getUnionId(), e), _.isObject(e.properties) && !_.isEmptyObject(e.properties) && _.extend(a.properties, e.properties), e.type && "profile" === e.type.slice(0, 7) || (za.para.batch_send && (a._track_id = Number(String(Math.random()).slice(2, 5) + String(Math.random()).slice(2, 4) + String(Date.now()).slice(-4))), a.properties = _.extend({}, _.info.properties, za.store.getProps(), _.info.currentProps, a.properties), a.properties.$is_first_day = _.getIsFirstDay()), a.properties.$time && _.isDate(a.properties.$time) ? (a.time = 1 * a.properties.$time, delete a.properties.$time) : za.para.use_client_time && (a.time = 1 * new Date), _.parseSuperProperties(a.properties), _.searchObjDate(a), _.searchObjString(a), logger.info(a), za.events.emit("send", a), za.sendStrategy.send(a)
}, za.store = {
